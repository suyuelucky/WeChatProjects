...
let ServiceContainer, StorageService, PhotoService, ImageCacheManager, MemoryManager, EventEmitter;
...
beforeAll(async () => {
  ...
  try {
    EventEmitter = await import('../miniprogram/utils/event-emitter.js').then(m => m.default || m);
  } catch (e) {
    // 如果找不到event-emitter.js，则使用eventBus.js
    try {
      EventEmitter = await import('../miniprogram/utils/eventBus.js').then(m => m.default || m);
    } catch (e2) {
      // 如果两者都找不到，则创建一个模拟对象
      EventEmitter = {
        events: {},
